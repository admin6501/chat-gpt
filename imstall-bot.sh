...
✅ اطلاع‌رسانی سفارش‌ها به ادمین
✅ ارسال رسید (متنی یا تصویری) برای ادمین
✅ بازگشت در همه مراحل کار می‌کند
"""
...
async def buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # اطلاع به ادمین
    msg = f"🆕 سفارش جدید ثبت شد:\n👤 {user.full_name} (@{user.username})\n🆔 سفارش #{oid}\n💰 مبلغ: {config['PRODUCT_PRICE']:,} تومان"
...
async def admin_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "✅ تایید پرداخت":
        await update.message.reply_text("شماره سفارش برای تایید پرداخت را بفرستید:", reply_markup=admin_menu())
