...
✅ پنل ادمین و کاربر کاملاً فعال
✅ دکمه بازگشت همه‌جا کار می‌کند
✅ مناسب برای اجرا روی VPS یا Termux
"""
...
async def back(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """بازگشت به منوی اصلی در هر حالت"""
...
async def admin_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "✅ تایید پرداخت":
        await update.message.reply_text("لطفاً شماره سفارش را ارسال کنید:", reply_markup=admin_menu())
    ...
    if context.user_data.get("admin_action") == "confirm":
        try:
            ...
            await update.message.reply_text("❌ شماره سفارش نامعتبر است.", reply_markup=admin_menu())
            return
        conn = sqlite3.connect(DB_FILE)
        c = conn.cursor()
        ...
        conn.commit()
        conn.close()
        await update.message.reply_text(f"✅ سفارش #{oid} تأیید شد.", reply_markup=admin_menu())
        context.user_data.clear()
        return
    ...
    if str(context.user_data.get("admin_action", "")).startswith("send_"):
        ...
        conn.commit()
        conn.close()
        await update.message.reply_text(f"📤 اکانت سفارش #{oid} ارسال شد:\n{email} | {password}", reply_markup=admin_menu())
        context.user_data.clear()
        return
