...
# -*- coding: utf-8 -*-
"""
ربات فروش اکانت ChatGPT — نسخه نهایی اصلاح‌شده
✅ دکمه‌های منوی کاربر کاملاً فعال
✅ دکمه برگشت بعد از خرید درست شد
✅ هیچ تداخلی با پنل ادمین ندارد
"""
...
async def my_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
    c.execute("SELECT id, status, price, created_at FROM orders WHERE user_id=?", (user.id,))
    rows = c.fetchall()
    conn.close()
    if not rows:
    ...
    msg = "📦 سفارش‌های شما:\n"
    for r in rows:
        msg += f"#{r[0]} | {r[2]:,} تومان | وضعیت: {r[1]} | {r[3][:16]}\n"
    await update.message.reply_text(msg, reply_markup=main_menu())

async def about(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(config.get("ABOUT_TEXT", "❌ هنوز متنی برای درباره محصول تنظیم نشده."), reply_markup=main_menu())

async def rules(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(config.get("RULES_TEXT", "❌ هنوز قوانین تنظیم نشده."), reply_markup=main_menu())

async def support(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(config.get("SUPPORT_TEXT", "❌ هنوز متن پشتیبانی تنظیم نشده."), reply_markup=main_menu())
