...
# -*- coding: utf-8 -*-
"""
ربات فروش اکانت ChatGPT — نسخه اصلاح‌شده نهایی
✅ پنل ادمین با /admin باز می‌شود
✅ دکمه‌های ارسال اکانت و تایید پرداخت فعال‌اند
✅ دکمه بازگشت به منوی اصلی از پنل ادمین به منوی کاربران می‌رود
"""
...
async def confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_CHAT_ID:
        return
    await update.message.reply_text("شماره سفارش برای تایید را وارد کنید:", reply_markup=admin_menu())
    context.user_data["action"] = "confirm"

async def send_account(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_CHAT_ID:
        return
    await update.message.reply_text("شماره سفارش برای ارسال اکانت را وارد کنید:", reply_markup=admin_menu())
...
async def admin_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # بازگشت از پنل ادمین → منوی کاربر
