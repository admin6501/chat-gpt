...
✅ ویژگی‌ها:
- پنل ادمین با دکمه‌های کامل (تایید پرداخت، ارسال اکانت، تنظیمات فروشگاه)
- نمایش خودکار پنل ادمین برای ادمین در /start
- دکمه بازگشت و منوهای جداگانه برای کاربر و ادمین
- لغو خودکار سفارش‌ها با زمان قابل تنظیم
"""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id == ADMIN_CHAT_ID:
        await update.message.reply_text("👑 خوش آمدید مدیر عزیز!", reply_markup=admin_menu())
        return
...
async def confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_CHAT_ID:
        return
    await update.message.reply_text("شماره سفارش را برای تایید بفرستید:", reply_markup=admin_menu())
    context.user_data["action"] = "confirm"

async def send_account(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != ADMIN_CHAT_ID:
        return
    await update.message.reply_text("شماره سفارش را برای ارسال اکانت بفرستید:", reply_markup=admin_menu())
...
async def admin_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("action") == "send_account":
        try:
            ...
            await update.message.reply_text("❌ شماره سفارش معتبر نیست.", reply_markup=admin_menu())
            return
        await update.message.reply_text("اکانت را به این فرمت ارسال کنید:\n`email@example.com | password123`", parse_mode="Markdown")
    ...
    if "set_key" in context.user_data:
        ...
        if key in ["PRODUCT_PRICE", "CANCEL_TIME_MINUTES"]:
            try:
                ...
                await update.message.reply_text("❌ فقط عدد وارد کنید.", reply_markup=settings_menu())
                return
        config[key] = value
        save_config(config)
